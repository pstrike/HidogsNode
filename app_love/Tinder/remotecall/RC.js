...
var RemoteCall = {
    getUser: function (userId) {
        var promise = new Promise(function(resolve, reject){
            var url = APIUtils.makeUrl("/user/"+userId+"?projection=user_id,nick_name,head_image_url,love,openid,pet");
    ...
    getUserList: function (userId) {
        var promise = new Promise(function(resolve, reject){
            var url = APIUtils.makeUrl("/love/other/tinder?userid="+userId+"&projection=user_id,pet,nick_name,head_image_url");
            APIUtils.get(url, function(result) {
                if(result.response == HidogsConstants.WEB_UTILS_REQUEST_TIMEOUT
                    || result.response == HidogsConstants.WEB_UTILS_REQUEST_NOT_FOUND
                    || result.response == HidogsConstants.WEB_UTILS_REQUEST_ERROR) {
    ...
    hateUser: function (userId, hatedUserId) {
        var promise = new Promise(function(resolve, reject){
            var url = APIUtils.makeUrl("/love/other/hateuser?userid="+userId+"&hateduserid="+hatedUserId);
            APIUtils.get(url, function(result) {
                if(result.response == HidogsConstants.WEB_UTILS_REQUEST_TIMEOUT
                    || result.response == HidogsConstants.WEB_UTILS_REQUEST_NOT_FOUND
                    || result.response == HidogsConstants.WEB_UTILS_REQUEST_ERROR) {
